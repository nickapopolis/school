	...
	implements Highlighter.HighlightPainter, CaretListener, MouseListener, MouseMotionListener
{
	...
	*/
	...
	public void selectText()
	{
		try
		{
			int linenum = ((JTextArea)component).getLineOfOffset(component.getCaretPosition());
			int startIndex = ((JTextArea)component).getLineStartOffset(linenum);
			int endIndex = ((JTextArea)component).getLineEndOffset(linenum);
			selectedText = component.getText().substring(startIndex, endIndex);
			if(!disabled)
			{
				StringTokenizer tk = new StringTokenizer(selectedText);
				selectedText = tk.nextToken("\n");
			}
		}
		catch(BadLocationException ble) {}
	}
